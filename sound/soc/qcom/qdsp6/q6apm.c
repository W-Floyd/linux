...
EXPORT_SYMBOL_GPL(q6apm_send_cmd_sync);

/*
 * EXPERIMENT: hooks for q6apm-voice-poc.ko, so the voice call proof of
 * concept can be reloaded without rebooting (snd-q6apm itself cannot be
 * unloaded: its GPR port would be lost). The map handle of a shared-memory
 * region mapped with token Q6APM_POC_MAP_TOKEN lands in q6apm_poc_map_handle.
 */
u32 q6apm_poc_map_handle;
EXPORT_SYMBOL_GPL(q6apm_poc_map_handle);

struct q6apm *q6apm_poc_get(void)
{
	return g_apm;
}
EXPORT_SYMBOL_GPL(q6apm_poc_get);
...
static int apm_callback(const struct gpr_resp_pkt *data, void *priv, int op)
{
	...
	case APM_CMD_RSP_SHARED_MEM_MAP_REGIONS:
		...
		if (hdr->token == Q6APM_POC_MAP_TOKEN) {	/* EXPERIMENT */
			q6apm_poc_map_handle = rsp->mem_map_handle;
			wake_up(&apm->wait);
			break;
		}
		info = idr_find(&apm->graph_info_idr, graph_id);
